...
class Logger {
  ...
  cart = {
    ...
    totals: (data: { subtotal: number; tax: number; total: number; itemCount: number }) => {
      this.debug(`Cart totals: ${data.itemCount} items, LKR ${data.total} (${data.subtotal} + ${data.tax} tax)`, {
        prefix: 'Cart',
        color: '\x1b[34m'
      })
    },
